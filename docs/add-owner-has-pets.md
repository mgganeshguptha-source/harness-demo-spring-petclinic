...
Add a convenience method named `hasPets()` to the `Owner` entity class.

- Returns `true` when the owner has one or more pets
- Returns `false` when the owner has no pets
- Reads the existing pets collection
- Does not add, remove, or modify pets
- Does not change how pets are stored or added
...
- **Out of scope:** UI, template, controller, or endpoint changes; any change to how pets are added or stored
